...
const getStaffCheckInMainOnDate=async (msg:Message,bot:TelegramBot,router:Router):Promise<void>=>{
    const chatId=msg.chat.id;
    const staffs=await staffService.getStaffsCheckInOnDateTypeShiftByBranchId('special','40375c41-7ba2-41ac-9fb7-9e78c1c6914b');
    if(staffs.length>0){
        const message=staffs.map((staff:any,index:number)=>`${index+1}. ${staff.full_name} - ${staff.company_email}`).join("\n");
        bot.sendMessage(chatId,message);
    }else{
...
const initStaffRoutes=(router:Router):void=>{
    ...
    router.addRoute("/staffs_checkin_main_current",(msg,bot)=>getStaffCheckInMainOnDate(msg,bot,router));
}
